...
def genHTML(_id, _pair):
    ...
    # 產生顏色
    colors = ['#cf03f5' if sel_df.loc[i, 'LongORShort'] == '做空'and sel_df.loc[i,'ProfitRate']>0  else (\
              '#f09af5' if sel_df.loc[i, 'LongORShort'] == '做空'and sel_df.loc[i,'ProfitRate']<=0 else (\
              '#2d63f2' if sel_df.loc[i, 'LongORShort'] == '做多'and sel_df.loc[i,'ProfitRate']>0  else  \
              '#85a2ed' ))  for i in range(len(sel_df))]
    sel_df['color'] = colors
    ...
    lose_open_dot = p.triangle(source=t_source, x='open_hour_interval', y='Open', color='color', size=10,
                                 line_alpha=0.5,
                                 fill_alpha=1, line_color='black', view=lose_view)
    # 平倉時間的點
